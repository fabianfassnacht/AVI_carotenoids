...
for(k in 1:36) { 
  ...
  for (j in j1:37) {
    ...
    for (h in h1:38) {  
      ...
      band1 <- maple_refl[,k]*100
      band2 <- maple_refl[,j]*100
      band3 <- maple_refl[,h]*100
      ...
      c1 <- lm(maple_chl~AVI1+I(AVI1^2))
      c1_sum <- summary(c1)
      
      c2 <- lm(maple_chl~AVI2+I(AVI2^2))
      c2_sum <- summary(c2)
      
      c3 <- lm(maple_chl~AVI3+I(AVI3^2))
      c3_sum <- summary(c3)

      
      if (c1_sum$adj.r.squared > 0.6) AVI_index <- paste("##### ##### AVI1: ", k, " ", j, " ", h)
      if (c1_sum$adj.r.squared > 0.6) print(AVI_index, attach=TRUE)
      if (c1_sum$adj.r.squared > 0.6) print(c1_sum$adj.r.squared, attach=TRUE)
      
      if (c2_sum$adj.r.squared > 0.6) AVI_index <- paste("##### ##### AVI2: ", k, " ", j, " ", h)
      if (c2_sum$adj.r.squared > 0.6) print(AVI_index, attach=TRUE)
      if (c2_sum$adj.r.squared > 0.6) print(c2_sum$adj.r.squared, attach=TRUE)
      
      if (c3_sum$adj.r.squared > 0.6) AVI_index <- paste("##### ##### AVI3: ", k, " ", j, " ", h)
      if (c3_sum$adj.r.squared > 0.6) print(AVI_index, attach=TRUE)
      if (c3_sum$adj.r.squared > 0.6) print(c3_sum$adj.r.squared, attach=TRUE)
...
#AVI2 <- AVI2-(refl[,38]^-1)
# fit quadratic model
c2qu <- lm(maple_chl~AVI2+I(AVI2^2))
...
rmse <- sqrt(sum(sumc2qu$residuals^2) / length(maple_chl))
rmse
...
tiff(filename = "Maple_1_13_14.tif",
