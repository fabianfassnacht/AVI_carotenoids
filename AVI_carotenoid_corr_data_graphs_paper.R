pkgs<-c("rgdal","caret","raster","foreign", "randomForest", "mgcv", "stats")
...
pigPlot4(car, AVI, species, filename="13_AVI_species_pch.pdf", indexname="AVI")
pigPlot4(car, git, species, filename="14_mCRI_species_pch.pdf", indexname="mCRI")
pigPlot4(car, chap, species, filename="15_CI_species_pch.pdf", indexname="CI")
pigPlot4(car, chap_AVI, species, filename="16_AVI_CI_species_pch.pdf", indexname="merged AVI CI")
pigPlot4(car, git_AVI, species, filename="17_AVI_mCRI_species_pch.pdf", indexname="merged mCRI AVI")
...
pigPlot4 <- function(car, index, species, filename, indexname) {
  ...
  # quadratic
  c1 <- lm(car ~  index + I(index^2))
  cq <- coef(c1)
  newenv2 = seq(min(index, na.rm=TRUE), max(index, na.rm=TRUE), by = (max(index, na.rm=TRUE) - min(index, na.rm=TRUE))/89)
  blu <- cq[1] + cq[2]*newenv2 +cq[3]*newenv2^2
  lines(blu, newenv2, col="blue")
    
  # linear
  c3 <- lm(car ~  index)
  cq3 <- coef(c3)
  newenv4 = seq(min(index, na.rm=TRUE), max(index, na.rm=TRUE), by = (max(index, na.rm=TRUE) - min(index, na.rm=TRUE))/89)
  blu3 <- cq3[1] + cq3[2]*newenv4
  lines(blu3, newenv4, col="green")
  
  dev.off()
  
  #exponential
  
#   c2 <- nls(car ~ a+b*I(1-exp(1)^(c * index)), start=list(a = 0, b= 1, c=1))
#   cq2 <- coef(c2)
#   newenv3 = seq(min(index, na.rm=TRUE), max(index, na.rm=TRUE), by = (max(index, na.rm=TRUE) - min(index, na.rm=TRUE))/89)
#   blu2 <- cq2[1]+cq2[2]*(1-exp(1)^(cq2[3]*newenv3))
#   lines(blu2, newenv3, col="red")
#   
}

dev.off()

spl_NE(car, AVI, add=F)
spl_NE(car, git)
spl_NE(car, chap)
spl_NE(car, chap_AVI)
spl_NE(car, git_AVI)
