...
setwd("D:/KIT_Forschung/11_AG_Angular_Index/99_Paper/1_AVI_search/SR/systematic_SR")
path = "SR_quad_r2_cv.txt"  
sink(path)
...
cv <- function(x,y,i,idx=NULL) {
  ...
  for (ii in 1:i) {
    ...
    linm <- lm(y_tr ~ x_tr + I(x_tr^2))
    cq <- coef(linm)      
    lm_pred <- cq[1] + cq[2]*x_te + cq[3]*x_te^2
